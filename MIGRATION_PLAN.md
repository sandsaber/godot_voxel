...
| Build time на CI | GitHub Actions | <15 мин incremental; ручной Rust workflow заготовлен 2026-07-07, auto-run временно выключен |
...
| 2026-07-07 | Rust workspace workflow | 🟡 `.github/workflows/rust.yml` заготовлен как manual-only (`workflow_dispatch`): pinned toolchain + Cargo cache + `cargo fmt --all -- --check`, `cargo test --workspace`, `cargo clippy --workspace --all-targets`, `cargo build --workspace`. Push/PR triggers временно выключены, чтобы не ломать GitHub flow |
| 2026-07-07 | Android aarch64 workflow smoke | 🟡 заготовлено в ручном workflow: `rust.yml` ставит NDK 29.0.14206865 и запускает `./scripts/android-build.sh` для `voxel-gdext` aarch64 `.so`. Локальный macOS smoke проходит; автоматический CI-gate, bench smoke и x86_64-android emulator smoke остаются отдельными infra-пунктами |
...
**Фаза 4 — storage/streaming + meshing pipeline + single-LOD paging terrain + VoxelEngine foundation/task loop работают headlessly (655 unit + 11 integration тестов).**
...
Audit wave 2D replaced that common data RwLock with a settings lock plus independent
per-LOD map locks, and migrated terrain/mesh worker paths onto the split-lock API.
...
2. **Фаза 4 — что осталось (после single-LOD paging terrain):**
   ...
   - **Concurrency audit follow-ups** — ThreadSanitizer coverage for the
     threaded edit/load/mesh path; macOS cargo stress is covered.
...
| 4.6o | `ThreadedTaskRunner` semaphore/staging + terrain nonblocking drain | `util/tasks/threaded_task_runner.cpp` (`_tasks_semaphore`, `_staged_tasks`), `terrain/fixed_lod/voxel_terrain.cpp` (`process`) | tasks, terrain | ✅ Audit-driven concurrency fix: enqueue stages tasks under a separate lock, workers wake through `thread::Semaphore`, cached-priority queue is sorted before end-pop, and `VoxelTerrainCore::process()` drains completed work without blocking a tick |
| 4.6 | `storage::voxel_data` | `storage/voxel_data.{h,cpp}` | VoxelDataMap, streams, generators, LOD | In progress: metadata, `VoxelDataGrid`, `try_set_block` with action_when_exists. `SharedVoxelData` worker bridge has settings lock + per-LOD map locks + `SpatialLock3D` regions |
...
cd rust
cargo test -p voxel-core       # 655 unit + 11 integration + 1 doc-test; 1 ignored diagnostic snapshot
