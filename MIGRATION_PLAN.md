...
| `generators::simple` | `generators/simple/{voxel_generator_waves,voxel_generator_flat}.{h,cpp}` | ✅ +14 тестов (`Waves` — синусоидальный heightmap `0.5+0.25*(cos+sin)`; `Flat` — плоскость на высоте Y с SDF/blocky путями и two early-exit ветками). Все значения параметров по value, `&mut` borrow вместо C++ RWLock. Тесты покрывают: height-функцию в опорных точках, bounded [0,1], pattern offset/size, SDF gradient + iso_scale, blocky fill, все early-exit ветки, used_channels_mask, heightmap range-remap через интеграцию с Waves |
...
cd rust
cargo test -p voxel-core       # 299 проходят (294 unit + 5 integration; +1 ignored golden-gen)
