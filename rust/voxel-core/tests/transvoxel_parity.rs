...
fn check_against_golden(golden_json: &str, inner: i32, radius: f32) {
    ...
    // The committed golden must come from the C++ baseline harness, not from
    // a Rust self-dump — a self-generated golden would make this test
    // circular and hide real parity bugs.
    assert_eq!(
        golden.generator, "godot_voxel-cpp",
        "committed golden was not generated by the C++ harness"
    );
