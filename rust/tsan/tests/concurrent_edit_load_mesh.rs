...
use voxel_core::meshers::{
    MeshBlockTask, MeshBlockTaskParams, MesherInput, MesherOutput, VoxelMesher,
};
...
fn concurrent_edit_load_mesh_keeps_data_and_region_locks_consistent() {
    ...
    let runner = ThreadedTaskRunner::new(6);
    ...
    thread::scope(|scope| {
        ...
        scope.spawn(move || {
            ...
            for i in 0..EDIT_OPS {
                ...
                let voxel_box = Box3i::new(pos, Vector3i::splat(1));
                let _write_region = edit_data.write_region(0, voxel_box);
                if edit_data.try_set_voxel(i as u64, pos, ChannelId::Type.index()) {
                    edit_data.mark_area_modified(voxel_box, false);
        ...
        scope.spawn(move || {
            ...
            for i in 0..LOAD_OPS {
                ...
                let voxel_box = Box3i::new(block_pos * block_size, Vector3i::splat(block_size));
                ...
                let _write_region = load_data.write_region(0, voxel_box);
                if load_data.try_set_block(block_pos, VoxelDataBlock::with_voxels(voxels, 0)) {
    ...
    let mut completed = runner.drain_completed_tasks();
    let mut mesh_outputs = 0;
    for task in &mut completed {
        let Some(task) = (task.as_mut() as &mut dyn Any).downcast_mut::<MeshBlockTask>() else {
            continue;
        };
